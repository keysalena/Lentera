...
<div class="admin-kemampuan" style="animation: fadeIn 0.4s ease-in-out;">
    ...
    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 32px; flex-wrap: wrap; gap: 20px;">
        <div>
            ...
            <p style="font-size: 14px; color: var(--ink-60);">Kelola parameter kompetensi bakat dan minat siswa untuk mapping evaluasi.</p>
        </div>
        
        <div style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">
            <form action="{{ route('admin.kemampuan') }}" method="GET" style="display: flex; gap: 12px; align-items: center; margin: 0;">
                <input type="text" name="cari" value="{{ request('cari') }}" placeholder="Cari indikator kemampuan..." 
        ...
        <table style="width: 100%; border-collapse: collapse;">
            <thead style="background: var(--cream);">
                <tr>
                    ...
                    <th style="padding: 16px 24px; text-align: left; font-size: 12px; color: var(--ink-60);">NAMA INDIKATOR KOMPETENSI</th>
            ...
            <tbody>
                ...
                <tr style="border-top: 1px solid var(--cream);">
                    ...
                    <td style="padding: 20px 24px;">
                        <div style="font-weight: 700; color: var(--ink); font-size: 15px;">{{ $kemampuan->nama_kemampuan }}</div>
                    </td>
                    <td style="padding: 20px 24px; text-align: right;">
                        <div style="display: flex; justify-content: flex-end; gap: 12px; align-items: center;">
                            <button onclick="openKemampuanModal('{{ $kemampuan->id_kemampuan }}', '{{ addslashes($kemampuan->nama_kemampuan) }}')" style="background:none; border:none; color: var(--ink-60); font-weight: 600; font-size: 13px; cursor: pointer; padding:0;">
                                Edit
                            </button>
                            <span style="color: var(--cream);">|</span>
                            
                            <form action="{{ route('admin.kemampuan.destroy', $kemampuan->id_kemampuan) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus indikator {{ $kemampuan->nama_kemampuan }}? Pengisian kuisioner siswa pada bidang ini akan ikut terhapus.');" style="margin: 0;">
                                @csrf
                                @method('DELETE')
        ...
        <div style="background: var(--white); width: 100%; max-width: 420px; border-radius: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.1); overflow: hidden; animation: fadeIn 0.3s ease-out;">
            ...
            <form id="kemampuanForm" action="" method="POST">
                ...
                <div style="padding: 32px; display: flex; flex-direction: column; gap: 20px;">
                    <div>
                        <label style="display: block; font-size: 13px; font-weight: 700; color: var(--ink); margin-bottom: 8px;">Nama Indikator Kemampuan</label>
                        <input type="text" id="inputNamaKemampuan" name="nama_kemampuan" required placeholder="Contoh: Kewirausahaan" style="width: 100%; padding: 12px 16px; border: 1px solid var(--ink-30); border-radius: 10px; font-size: 14px; outline: none;">
                    </div>
                </div>
...
<script>
    ...
    function openKeepModal(id = null, nama_kemampuan = '') {
        // Alias fungsi agar fleksibel dipanggil
        openKemampuanModal(id, nama_kemampuan);
    }

    function openKemampuanModal(id = null, nama_kemampuan = '') {
        if(id) {
